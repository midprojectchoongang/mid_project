...
<c:if test="${not empty commList }">
	...
	<c:forEach var="comments" items="${commList }">
		...
		<c:if test="${not empty member_id && comments.del == 'n'}">
			...
			<input type="button" id="updateComm" value="수정" class="btn-two comm charcoal rounded"
			onclick="cUpdate(${comments.comment_no})">
			<input type="button" id="deleteComm" value="삭제" class="btn-two comm charcoal rounded"
